...
const ALLOWED_EXTENSIONS = ['pdf', 'docx', 'doc', 'pptx', 'ppt', 'xlsx', 'xls', 'csv'];
...
const buildAnalysisSystemPrompt = ({ ext, wasTruncated, truncatedText, memoryContext, userMessage }) => `
...
## 🗺️ Action Plan
A prioritized 30 / 60 / 90-day action plan with specific next steps.
  ...
  try {
    ...
    if (!extractedText || extractedText.trim().length < 30) {
      return res.status(422).json({
        message: 'Could not extract readable text. The file may be image-based or empty.'
      });
    }
    ...
    const systemPrompt = buildAnalysisSystemPrompt({
      ext, wasTruncated, truncatedText, memoryContext, userMessage
    });

    // 4. Stream AI analysis
    if (!process.env.GEMINI_API_KEY) {
      res.write(`data: ${JSON.stringify({ type: 'error', content: 'GEMINI_API_KEY not configured.' })}\n\n`);
      return res.end();
    }
    ...
    const stream = await llm.stream([
      ...
      new HumanMessage('Please analyze this document and provide your expert business consultant report.')
    ]);
    ...
    res.write(`data: ${JSON.stringify({ type: 'document_saved', documentId: document._id.toString(), title: docTitle })}\n\n`);
...
const askAboutDocument = async (req, res) => {
  try {
    ...
    const history = doc.chatHistory.slice(-10).map(msg =>
